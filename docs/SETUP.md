...
- **Redis（可选）**：默认 `REDIS_ENABLED=false`，单 worker 本地开发**不需要开启**；
  ...
  启用后登录失败计数、摄入任务进度、BM25 文档集签名、embedding 向量缓存与
  检索结果 identity 缓存跨 worker 共享；
  关闭时分别回退为进程内实现。
...
- Alembic 迁移全部业务表（users / sessions / messages / documents / tasks / app_settings / token_sessions）
