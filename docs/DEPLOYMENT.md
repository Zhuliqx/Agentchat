...
| 登录失败计数（5 分钟窗口） | `app/api/routes/auth.py` | Redis 未启用时每 worker 独立计数，防爆破可被分散请求绕过 | 默认安全弱化；`REDIS_ENABLED=true` 后共享计数 |
...
> （登录失败计数、摄入任务进度、文档集签名、embedding 向量缓存与
> 检索结果 identity 缓存已自动共享）。以下状态无需迁移或保持取舍：
