...
def ingest_file(
    ...
    - 内容与目标索引都与已有块一致的块 → 保留（不重复嵌入/写入）；
      内容相同但索引发生平移的块会按新索引重写，避免唯一键冲突；
    - 新增/变化的块 → 嵌入并写入；
    - 旧文档中已消失的块 → 从 Postgres 与 Milvus 删除；
    - 整篇无变化 → 直接返回 unchanged=True（不触碰任何数据）。
    一致性：解析/嵌入先于任何库变更；Postgres 的过期行删除与新行插入在同一个
    事务内（失败整体回滚）；Milvus 同步失败时新行保持 pending，由对账任务补同步。

    force_reingest：True 时跳过增量，嵌入后在同一事务中删除旧块并全量重建。
