...
2. 每个块头标注了它在本轮检索里的相关性名次（如「本次第 1 位」），优先采信名次靠前的块；
   块的先后只保证「来源编号」连续，不代表相关性。多条内容一致时综合归纳，信息冲突时如实说明差异。
3. 如果检索结果不足以回答，明确告知用户"知识库中没有相关信息"，不要猜测或编造。
4. 回答使用中文，条理清晰；引用某段信息时在该句末尾标注来源编号，如「[1]」「[2]」——
   编号对应检索结果里的【来源 N】。同一句可标注多个编号，界面会把编号与来源列表联动。
...
3. 执行环境为隔离容器：无网络、根文件系统只读、有 CPU/内存/PID 限额；
   仅 /tmp 可写；代码会超时（默认 15s）并截断输出。优先只做纯计算，
   不要依赖宿主机文件或外部服务。
...
def build_supervisor_prompt(
    ...
    rn = 1
    rules.append(
        f"{rn}. 回答一律用中文：正文直接进入结论，不要输出\"I'll search for…\"这类英文过程性开场白，"
        "调用工具前的说明也必须是中文。"
    )
    rn += 1
    if use_rag:
        rules.append(
            ...
            "绝不要用 mcp_agent 去查数据库代替；引用其中内容时保留来源编号（如 [1]），"
            "不要改写成文件名——界面用编号与来源列表联动。"
        )
    else:
        rules.append(
    ...
    rn += 1
    rules.append(
        f"{rn}. 直接给结果：不要输出“我来搜索/让我查一下/I'll search…”这类过程性说明、"
        "英文寒暄或中英夹杂的开场白，工具调用过程由界面单独展示；"
        "最终回答使用与用户提问一致的语言（默认中文）。"
    )
    rn += 1
