  ...
  <Modal :open="open" title="定时任务" @close="open = false">
    ...
    >
      ...
      <label class="flex flex-col gap-1.5 text-[12px] text-ink-faint">
        ...
        <input
          v-model="form.name"
          class="rounded-lg border border-line-2 bg-surface px-3 py-1.5 text-[13px] text-ink outline-none placeholder:text-ink-faint focus:border-accent"
      ...
      <label class="flex flex-col gap-1.5 text-[12px] text-ink-faint">
        ...
        <select
          ...
          class="rounded-lg border border-line-2 bg-surface px-3 py-1.5 text-[13px] text-ink outline-none focus:border-accent"
        >
      ...
      <label class="flex flex-col gap-1.5 text-[12px] text-ink-faint">
        ...
        <input
          ...
          class="rounded-lg border border-line-2 bg-surface px-3 py-1.5 font-mono text-[13px] text-ink outline-none focus:border-accent"
          placeholder="interval:3600 或 cron:*/30"
        />
        <small class="text-[10.5px] text-ink-faint"
          >interval:&lt;秒&gt; 固定间隔；cron:&lt;分钟&gt; 分钟级（如 */30、0）</small
        >
      </label>
      <div class="flex gap-2">
        <button
          class="rounded-lg bg-accent px-3.5 py-1.5 text-[12px] font-medium text-white transition hover:brightness-110"
          @click="saveTask"
        >
          保存
        </button>
        <button
          class="rounded-lg border border-line-2 px-3.5 py-1.5 text-[12px] text-ink-dim transition hover:text-ink"
    ...
    <div class="flex max-h-[55vh] flex-col gap-2 overflow-y-auto pr-1">
      ...
      >
        <div class="flex flex-wrap items-center gap-2">
          ...
          <span
            class="rounded-full bg-accent/12 px-2 py-0.5 text-[10.5px] font-medium text-accent"
            >{{ t.task_label }}</span
          >
          <span
            class="ml-auto flex items-center gap-1 text-[10.5px]"
        ...
        <div v-if="t.last_error" class="mt-1 break-all text-[11.5px] text-err">
          {{ t.last_error }}
        </div>
        <div class="mt-2.5 flex flex-wrap gap-1.5">
          ...
          <button
            class="flex items-center gap-1 rounded-md border border-err/30 px-2 py-1 text-[11px] text-err transition hover:bg-err/10"
