  ...
  <Modal :open="store.open" title="自主任务 Agent" @close="store.closeModal()">
    <div class="flex flex-col gap-3">
      ...
      <div class="rounded-xl border border-line bg-surface-2 p-3">
        <label class="mb-1.5 block text-xs text-ink-faint">目标（模糊长任务）</label>
        <textarea
          v-model="goal"
          rows="2"
          class="w-full resize-y rounded-lg border border-line-2 bg-surface px-3 py-2 text-sm leading-relaxed text-ink outline-none transition placeholder:text-ink-faint focus:border-accent focus:ring-2 focus:ring-accent/15"
          placeholder="如：调研 RAG 主流方案，对比优缺点并给出选型建议…"
          @keydown.enter.exact.prevent="start"
        />
        <div class="mt-2 flex items-center justify-between gap-2">
          <span class="text-2xs" :class="statusCls">{{ statusText }}</span>
          <div class="flex gap-2">
            <button
              v-if="store.running"
              class="flex items-center gap-1.5 rounded-lg border border-err/40 px-3 py-1.5 text-xs text-err transition hover:bg-err/10"
              @click="store.stop()"
            >
            ...
            <button
              v-else
              class="flex items-center gap-1.5 rounded-lg bg-accent px-3.5 py-1.5 text-xs font-medium text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
      ...
      <div v-if="store.events.length" class="rounded-xl border border-line p-3">
        <div class="mb-2 text-2xs font-medium uppercase tracking-wide text-ink-faint">执行轨迹</div>
        <div class="flex max-h-[180px] flex-col gap-1.5 overflow-y-auto pr-1">
          <div v-for="t in store.events" :key="t.id" class="flex items-start gap-2 text-xs">
            <Icon
            ...
            <span class="flex-shrink-0 text-2xs text-ink-faint">{{ t.time }}</span>
          </div>
        </div>
      </div>
      ...
      >
        <div class="mb-2 flex items-start gap-2 text-sm text-ink">
          <Icon name="warn" :size="15" class="mt-0.5 flex-shrink-0 text-warn" />
          <span class="min-w-0 flex-1">
            确认下一步执行？
            <span class="mt-0.5 block break-all text-xs text-ink-dim">
              {{ store.pending.next_action || "（无动作）" }}
            </span>
            <span
              v-if="store.pending.expected_source"
              class="mt-1 inline-block rounded bg-surface-3 px-1.5 py-0.5 text-2xs text-ink-faint"
            >
              信息源：{{
        ...
        <div v-if="editMode" class="mt-2 flex flex-col gap-2">
          <input
            v-model="editAction"
            class="h-8 w-full rounded-lg border border-line-2 bg-surface px-2.5 text-xs text-ink outline-none focus:border-accent"
            placeholder="替换后的动作"
          />
          <div class="flex items-center gap-2">
            <select
              v-model="editSource"
              class="h-8 flex-1 rounded-lg border border-line-2 bg-surface px-2 text-xs text-ink outline-none focus:border-accent"
            >
            ...
            <button
              class="h-8 rounded-lg bg-accent px-3 text-xs font-medium text-white transition hover:brightness-110 disabled:opacity-40"
            ...
            <button
              class="h-8 rounded-lg border border-line-2 px-3 text-xs text-ink-dim transition hover:text-ink"
        ...
        <div v-else class="mt-2 flex flex-wrap gap-2">
          ...
          <button
            class="rounded-lg border border-line-2 px-3.5 py-1.5 text-xs text-ink-dim transition hover:border-accent/50 hover:text-ink"
            @click="beginEdit"
          >
            编辑动作
          </button>
      ...
      >
        <div v-if="store.plan" class="rounded-xl border border-line p-3">
          <div class="mb-1.5 text-2xs font-medium uppercase tracking-wide text-ink-faint">计划</div>
          <div class="md text-xs" v-html="md.render(store.plan)" />
        </div>
        <div v-if="store.findings.length" class="rounded-xl border border-line p-3">
          <div class="mb-1.5 text-2xs font-medium uppercase tracking-wide text-ink-faint">
          ...
          <div class="flex max-h-[160px] flex-col gap-1 overflow-y-auto pr-1">
            <div
              ...
              :key="i"
              class="flex items-start gap-1.5 text-xs text-ink-dim"
            >
              <span class="mt-px flex-shrink-0 font-mono text-2xs text-ink-faint"
                >{{ i + 1 }}.</span
              >
        ...
        <div v-if="store.finalAnswer" class="rounded-xl border border-line bg-surface-2/40 p-3">
          <div class="mb-1.5 flex items-center justify-between">
            <span class="text-2xs font-medium uppercase tracking-wide text-ink-faint"
              >最终答案</span
            >
            <button
          ...
          <div class="md text-sm" v-html="md.render(store.finalAnswer)" />
        </div>
      </div>

      <div v-if="store.error" class="rounded-lg bg-err/10 px-3 py-2 text-xs text-err">
        {{ store.error }}
      </div>
      ...
      <div v-if="store.sessionId" class="rounded-xl border border-line">
        <div class="flex items-center justify-between border-b border-line px-3 py-2">
          <span class="flex items-center gap-1.5 text-xs font-medium text-ink-dim">
            <Icon name="clock" :size="13" class="text-ink-faint" />
            版本历史
          </span>
          <button
            class="flex items-center gap-1 rounded-md border border-line-2 px-2 py-1 text-2xs text-ink-dim transition hover:border-accent/50 hover:text-ink"
        ...
        <div v-if="store.history.length" class="max-h-[220px] overflow-y-auto p-2">
          ...
          >
            <div class="flex w-4 flex-shrink-0 flex-col items-center">
              <span
                class="mt-1 grid h-4 w-4 place-items-center rounded-full border border-line-2 bg-surface-2 text-2xs font-medium text-ink-faint"
              >
            ...
            <div class="min-w-0 flex-1 pb-1.5">
              <div class="flex flex-wrap items-center gap-1.5 text-2xs">
                ...
                <span
                  v-if="c.interrupted"
                  class="flex items-center gap-0.5 rounded-full bg-warn/10 px-1.5 py-px text-2xs text-warn"
                >
                ...
                <span
                  ...
                  class="rounded-full bg-orbit/10 px-1.5 py-px text-2xs text-orbit"
                >
                  下一步：{{ c.next.join(", ") }}
                </span>
                <span v-else class="rounded-full bg-ok/10 px-1.5 py-px text-2xs text-ok">完成</span>
              </div>
              <div class="mt-0.5 line-clamp-2 break-all text-2xs text-ink-dim">
                {{ c.summary || "（无文本步骤）" }}
              </div>
              <div class="mt-1 flex items-center gap-1.5">
                <input
                  v-model="forkGoals[c.checkpoint_id]"
                  class="h-6 min-w-0 flex-1 rounded-md border border-line-2 bg-surface-2 px-1.5 text-2xs text-ink outline-none placeholder:text-ink-faint focus:border-accent"
                ...
                />
                <button
                  class="h-6 flex-shrink-0 rounded-md border border-line-2 px-1.5 text-2xs text-ink-dim transition hover:border-accent/50 hover:text-ink"
                  @click="
                    store.fork(c.checkpoint_id, c.checkpoint_ns || '', forkGoals[c.checkpoint_id])
                  "
                >
