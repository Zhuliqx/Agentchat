...
const { loading, error, reload } = useDialogResource(open, load);
...
<template>
  <Modal :open="open" title="管理后台" @close="open = false">
    <DialogShell :loading="loading && !stats" :loading-rows="5">
      <div v-if="error" class="mb-3 rounded-lg bg-err/10 px-3 py-2 text-xs text-err">
        {{ error }}
      </div>
      <div v-if="stats" class="mb-4 grid grid-cols-4 gap-2">
        <div class="rounded-lg border border-line bg-surface-2 px-1 py-2.5 text-center">
          <b class="block text-lg font-semibold text-ink">{{ stats.user_count }}</b>
          <span class="text-2xs text-ink-faint">用户</span>
        </div>
        <div class="rounded-lg border border-line bg-surface-2 px-1 py-2.5 text-center">
          <b class="block text-lg font-semibold text-ink">{{ stats.session_count }}</b>
          <span class="text-2xs text-ink-faint">会话</span>
        </div>
        <div class="rounded-lg border border-line bg-surface-2 px-1 py-2.5 text-center">
          <b class="block text-lg font-semibold text-ink">{{ stats.message_count }}</b>
          <span class="text-2xs text-ink-faint">消息</span>
        </div>
        <div class="rounded-lg border border-line bg-surface-2 px-1 py-2.5 text-center">
          <b class="block text-lg font-semibold text-ink">{{ stats.document_count }}</b>
          <span class="text-2xs text-ink-faint">文档块</span>
        </div>
      </div>
      ...
      <div v-if="usage" class="mb-4 rounded-lg border border-line p-3">
        ...
        <div class="flex h-24 items-end justify-between gap-2">
          ...
          >
            <span
              class="text-2xs font-semibold tabular-nums text-accent opacity-0 transition group-hover:opacity-100"
            >
              {{ u.messages }}
            </span>
            <div
            ...
            />
            <span class="text-2xs text-ink-faint transition group-hover:text-ink-dim">
              {{ u.date.slice(5) }}
            </span>
          </div>
        </div>
      ...
      <div v-if="settingsItems" class="mb-4 overflow-hidden rounded-lg border border-line">
        <div
          class="flex items-center justify-between border-b border-line bg-surface-2/40 px-3 py-2"
        >
          <button
            class="flex items-center gap-1.5 text-xs font-medium text-ink-dim transition hover:text-ink"
          ...
          <button
            ...
            class="rounded-md bg-accent px-2.5 py-1 text-2xs font-medium text-white transition hover:brightness-110"
            @click="saveSettings"
          >
            保存
          </button>
        ...
        <div v-if="!foldedPanel.settings" class="flex flex-col gap-2 p-3">
          <label
            ...
            :key="it.key"
            class="flex items-center justify-between gap-2 text-xs text-ink-dim"
          >
            <span>{{ it.label }}</span>
            <input
              v-if="it.type === 'bool'"
            ...
            <div v-else class="group relative flex items-center">
              <input
                ...
                class="h-7 w-12 rounded-md border border-line-2 bg-surface px-2 text-center text-xs text-ink outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/10 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
      ...
      <div class="mb-4 overflow-hidden rounded-lg border border-line">
        ...
        >
          ...
          <button
            ...
            class="flex items-center gap-1 rounded-md bg-accent px-2.5 py-1 text-2xs font-medium text-white shadow-sm transition hover:brightness-110 disabled:opacity-50"
        ...
        <div v-if="!foldedPanel.eval" class="flex flex-col gap-3 p-3">
          <div v-if="evalError" class="rounded-md bg-err/10 px-2 py-1.5 text-2xs text-err">
            {{ evalError }}
          </div>
          ...
          <div v-if="evalData?.docs.length">
            <div
              class="mb-1.5 flex items-center gap-1 text-2xs font-medium uppercase tracking-wide text-ink-faint"
            >
              知识库文档
              <span class="rounded bg-surface-3 px-1 py-px text-2xs font-normal">{{
            ...
            <div class="flex flex-wrap gap-1">
              <span
                ...
                :key="d.source"
                class="flex items-center gap-1 rounded-md border border-line bg-surface-2 px-1.5 py-0.5 text-2xs text-ink-dim"
              >
          ...
          <div v-if="evalData?.auto.length">
            ...
            >
              自动适配案例
              <span class="rounded bg-accent/10 px-1 py-px text-2xs font-normal text-accent">{{
            ...
            >
              <div
                ...
                :key="i"
                class="flex items-center gap-1.5 py-0.5 text-2xs text-ink-dim"
              >
                <Icon name="sparkle" :size="10" class="flex-shrink-0 text-accent/70" />
                <span class="truncate">「{{ c.query }}」</span>
                <span
                  class="ml-auto flex-shrink-0 truncate rounded bg-surface-3 px-1 py-px text-2xs text-ink-faint"
                  >{{ c.doc }}</span
                >
              </div>
            </div>
          ...
          <div>
            ...
            <div class="flex flex-col gap-1">
              <div v-for="(c, i) in customEdit" :key="i" class="flex items-center gap-1">
                <input
                  v-model="c.query"
                  class="h-6 min-w-0 flex-1 rounded-md border border-line-2 bg-surface px-1.5 text-2xs text-ink outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/10"
                  placeholder="问题"
                />
                <input
                  v-model="c.keywordsText"
                  class="h-6 w-[46%] rounded-md border border-line-2 bg-surface px-1.5 text-2xs text-ink outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/10"
              ...
              <div class="flex items-center gap-1">
                <input
                  v-model="newQuery"
                  class="h-6 min-w-0 flex-1 rounded-md border border-dashed border-line-2 bg-surface px-1.5 text-2xs text-ink outline-none transition placeholder:text-ink-faint focus:border-accent"
                  placeholder="新问题…"
                  @keyup.enter="addCustom"
                />
                <input
                  v-model="newKeywords"
                  class="h-6 w-[46%] rounded-md border border-dashed border-line-2 bg-surface px-1.5 text-2xs text-ink outline-none transition placeholder:text-ink-faint focus:border-accent"
                  placeholder="关键词…"
                  @keyup.enter="addCustom"
                />
                <button
            ...
            <button
              ...
              class="mt-1.5 rounded-md border border-line-2 px-2 py-0.5 text-2xs text-ink-dim transition hover:border-accent/50 hover:text-ink"
              @click="saveCustom"
            >
              保存自定义案例
            </button>
          ...
          <div v-if="evalResults" class="rounded-md border border-line bg-surface-2/30 p-2">
            <div
              ...
              :key="i"
              class="flex items-center gap-1.5 py-1 text-2xs"
            >
              <span
                class="grid h-4 w-4 flex-shrink-0 place-items-center rounded-full text-2xs font-bold"
              ...
              <span
                v-if="r.doc"
                class="max-w-[30%] flex-shrink-0 truncate text-2xs text-ink-faint"
                >{{ r.doc }}</span
              >
              <span class="flex-shrink-0 rounded bg-surface-3 px-1 py-px text-2xs text-ink-faint"
                >{{ r.hits }} 条</span
              >
            </div>
          ...
          <div
            ...
            class="rounded-md border border-dashed border-line px-2 py-2.5 text-center text-2xs text-ink-faint"
          >
      ...
      <div class="mb-4 overflow-hidden rounded-lg border border-line">
        ...
        >
          ...
          <span v-if="!foldedPanel.users" class="text-2xs text-ink-faint"
            >{{ users.length }} 个</span
          >
        </div>
        <div v-if="!foldedPanel.users" class="flex flex-col gap-px p-2">
          <div
            v-for="u in users"
            :key="u.id"
            class="flex items-center gap-2.5 rounded-md px-1.5 py-2 text-xs transition hover:bg-surface-2"
          >
            <span
              class="grid h-7 w-7 flex-shrink-0 place-items-center rounded-full text-2xs font-semibold"
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-1.5">
                ...
                <span
                  v-if="u.is_admin"
                  class="rounded-full bg-accent/12 px-1.5 py-px text-2xs font-medium text-accent"
                  >管理员</span
                >
                <span
                  v-if="u.id === auth.user?.id"
                  class="rounded-full bg-ok/12 px-1.5 py-px text-2xs font-medium text-ok"
                  >我</span
                >
              </div>
              <div class="mt-0.5 text-2xs text-ink-faint">
                会话 {{ u.session_count }} · 消息 {{ u.message_count }} · 文档
                {{ u.document_count }}
              </div>
            </div>
