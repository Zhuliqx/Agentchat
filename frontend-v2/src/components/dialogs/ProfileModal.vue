...
<template>
  <Modal :open="open" title="个人主页" @close="open = false">
    <DialogShell :loading="loading" :error="error" :loading-rows="6">
      <div v-if="st" class="flex flex-col gap-4">
        <!-- 横幅 -->
        <div
          class="flex items-center gap-3.5 rounded-xl border border-line bg-surface-2 px-4 py-3.5"
        >
          <div
          ...
          <div class="min-w-0">
            <div class="flex items-center gap-2">
              <span class="text-lg font-semibold tracking-tight">{{ st.username }}</span>
              <span class="rounded-full bg-ok/12 px-2 py-0.5 text-2xs font-medium text-ok"
                >已登录</span
              >
            </div>
            <div class="mt-0.5 truncate font-mono text-2xs text-ink-faint">{{ me?.id }}</div>
          </div>
        </div>
        <!-- 统计卡 -->
        <div class="grid grid-cols-4 gap-2">
          <div class="rounded-lg border border-line bg-surface-2 px-1 py-2.5 text-center">
            <b class="block text-xl font-semibold text-ink">{{ st.session_count }}</b>
          ...
          <div class="rounded-lg border border-line bg-surface-2 px-1 py-2.5 text-center">
            <b class="block text-xl font-semibold text-ink">{{ st.memory_count }}</b>
            <span class="text-2xs text-ink-faint">记忆</span>
          </div>
          <div class="rounded-lg border border-line bg-surface-2 px-1 py-2.5 text-center">
            <b class="block text-xl font-semibold text-ink">{{ st.document_count }}</b>
            <span class="text-2xs text-ink-faint">文档</span>
          </div>
        </div>
        <!-- 详情 -->
        <div class="overflow-hidden rounded-lg border border-line">
          <div class="flex items-center gap-2.5 bg-surface-2 px-3.5 py-2.5">
            ...
            <span class="flex-1 text-xs text-ink-faint">注册时间</span>
            <span class="text-xs text-ink-dim">{{ new Date(st.created_at).toLocaleString() }}</span>
          </div>
          <div class="flex items-center gap-2.5 border-t border-line bg-surface-2 px-3.5 py-2.5">
            ...
            <span class="flex-1 text-xs text-ink-faint">用户 ID</span>
            <span class="max-w-[220px] break-all font-mono text-2xs text-ink-dim">{{
              me?.id
            }}</span>
          </div>
          <div class="flex items-center gap-2.5 border-t border-line bg-surface-2 px-3.5 py-2.5">
            <Icon name="stats" :size="14" class="flex-shrink-0 text-ink-faint" />
            <span class="flex-1 text-xs text-ink-faint">内容估算</span>
        ...
        <div class="overflow-hidden rounded-xl border border-line">
          <div class="flex items-center gap-2 border-b border-line bg-surface-2 px-4 py-2.5">
            ...
            <span class="text-xs font-medium">资料</span>
          </div>
          <div class="flex flex-col divide-y divide-line">
            <!-- 头像颜色 -->
            <div class="p-4">
              <label class="flex items-center gap-1.5 text-xs font-medium text-ink">
            ...
            <div class="p-4">
              ...
              <p class="mt-1 text-2xs text-ink-faint">当前用户名：{{ st.username }}</p>
              <div class="mt-2.5 flex items-center gap-2">
                <input
                  v-model="profileForm.username"
                  placeholder="输入新用户名"
                  maxlength="32"
                  class="h-9 min-w-0 flex-1 rounded-lg border border-line-2 bg-surface px-3 text-sm text-ink outline-none transition placeholder:text-ink-faint focus:border-accent focus:ring-2 focus:ring-accent/15"
                />
                <button
                  :disabled="saving || !profileForm.username.trim()"
                  class="h-9 shrink-0 rounded-lg bg-accent px-4 text-sm font-medium text-white transition hover:brightness-110 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-40"
                  @click="saveProfile"
                >
                  {{ saving ? "保存中…" : "保存" }}
                </button>
        ...
        <div class="overflow-hidden rounded-xl border border-line">
          ...
          <div class="p-4">
            ...
            <div class="mt-2.5 flex flex-col gap-2">
              <div class="relative">
                <input
                  ...
                  placeholder="当前密码"
                  maxlength="128"
                  class="h-9 w-full rounded-lg border border-line-2 bg-surface pl-3 pr-9 text-sm text-ink outline-none transition placeholder:text-ink-faint focus:border-accent focus:ring-2 focus:ring-accent/15"
                />
                <button
                  type="button"
            ...
            <button
              ...
              class="mt-3 h-9 w-full rounded-lg bg-accent text-sm font-medium text-white transition hover:brightness-110 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-40"
              @click="savePassword"
            >
              {{ saving ? "保存中…" : "更新密码" }}
            </button>
        ...
        <div class="overflow-hidden rounded-xl border border-line">
          <div class="flex items-center gap-2 border-b border-line bg-surface-2 px-4 py-2.5">
            ...
            <span class="text-xs font-medium">数据与账号</span>
          </div>
          <div class="flex flex-col gap-2.5 p-4">
            <button
              class="flex h-9 items-center justify-center gap-2 rounded-lg border border-line-2 text-xs text-ink-dim transition hover:border-accent/50 hover:text-ink"
              @click="exportData"
            >
            ...
            <button
              class="flex h-9 items-center justify-center gap-2 rounded-lg border border-err/30 text-xs text-err transition hover:bg-err/10"
              @click="deleteAccount"
            >
        ...
        <div
          v-if="formMsg"
          class="flex items-center gap-2 rounded-xl border border-line px-4 py-2.5 text-xs transition"
